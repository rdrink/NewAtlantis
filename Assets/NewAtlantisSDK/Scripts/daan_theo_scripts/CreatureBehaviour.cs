...
public class CreatureBehaviour : MonoBehaviour {
	...
	void Start () {
		...
		this.rigidbody.MovePosition(this.transform.parent.transform.position + startPosition);
	...
	void Update () {
		...
		if( tmpDistance > tmp.domeDiameter/2 || tmpDistance < 12 ){ // normal behaviour (circle around center)
			...
			if( Vector3.Distance( targetPosition, this.transform.position ) > 5 ) this.rigidbody.AddForce( (targetPosition-this.transform.position)/5 , ForceMode.Force );
			// else get to camera
			else this.rigidbody.AddForce( this.rigidbody.velocity/10 , ForceMode.Force );
			//this.rigidbody.MovePosition(this.transform.position + (targetPosition-this.transform.localPosition)/5);
		}
